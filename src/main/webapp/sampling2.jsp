...
<%
		...
		String lastChr=prjName.substring(size-2);
		...
		if(lastChr.contains("FP")|| lastChr.contains("BP")){
...
<% 
			...
			String tlcodecheckpoint1="";
			String gendercheckpoint1="";
			String occupationcheckpoint1="";
			
			String prjNamefp=prjName.substring(0,size-2)+"FP";
			String prjNamebp=prjName.substring(0,size-2)+"BP";
			...
			try {
				 ...
				 ps=con.prepareStatement("SELECT DISTINCT(CHECKLIST_ID) FROM `CHECKLIST_MASTER` WHERE CHECKLIST_NAME IN (?,?)");//Fetched Check List for the selected Project
				 ps.setString(1, prjNamefp);
				 ps.setString(2, prjNamebp);
				 rs=ps.executeQuery();
				 String newId="(";
				 while(rs.next()){					 
			...
			//	 ps=con.prepareStatement("SELECT id  FROM `mainapp_project` WHERE `name` LIKE ? ");
				 ...
				 ps=con.prepareStatement("SELECT id  FROM `mainapp_project` WHERE `capi_checklist_id` IN "+newId);
				...
				//fetching fr,gender,occupation from survey genius BP
				 ps=con.prepareStatement("SELECT *  FROM `mainapp_projectspecificcheckpoints` WHERE `project_id`=? ORDER BY `id` DESC");
				 ps.setString(1, idd[1]);
				 rs=ps.executeQuery();

				 while(rs.next()){
					 tlcodecheckpoint1=rs.getString("tlcodecheckpoint");
					 gendercheckpoint1=rs.getString("gendercheckpoint");
					 occupationcheckpoint1=rs.getString("occupationcheckpoint");
				 }				 
				 rs.close();
				 ps.close();
				 ...
				 //System.out.println(tlcodecheckpoint+" "+tlcodecheckpoint1);
				// System.out.println(gendercheckpoint+" "+gendercheckpoint1);
				// System.out.println(occupationcheckpoint+" "+occupationcheckpoint1);
			...
			try {
				...
				// System.out.println(dontKnowId);
				 ...
				 String tlTemp="("+tlcodecheckpoint+","+tlcodecheckpoint1+")";
				 System.out.println("tlcheckpoint"+tlTemp);
				 
				 ps=con.prepareStatement("SELECT *  FROM `SAVE_SURVEY` WHERE `DATETIME` >=? AND  `DATETIME` <= ? AND CHECKPOINT_ID IN "+tlTemp);
			     ps.setString(1,date);
				...
				// System.out.println(frNameList); //fr name array
				 ...
				 for(int i=0;i<frNameList.size();i++){
					...
					// System.out.println(endDate);
					 ps=con.prepareStatement("SELECT id1 FROM `SAVE_SURVEY` WHERE `DATETIME` >=? AND  `DATETIME` <= ? AND CHECKPOINT_ID IN "+tlTemp+" AND VALUE=?");					 
				     ps.setString(1,date);
				     ps.setString(2,endDate);					
					 ps.setString(3,currentFrName);
					 rs=ps.executeQuery();
					 ...
					 String gchkpoint="("+gendercheckpoint+","+gendercheckpoint1+")";
					 ps=con.prepareStatement("SELECT COUNT(VALUE) FROM `SAVE_SURVEY` WHERE `CHECKPOINT_ID` IN "+gchkpoint+" AND `VALUE` LIKE '%2-FEMALE%' AND `id1` IN" +tempInterview);
					 //ps.setString(1,gchkpoint);		  
					 rs=ps.executeQuery();
