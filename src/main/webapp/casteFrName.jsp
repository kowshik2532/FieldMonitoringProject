...
<body>
     ...
     try {
     	Class.forName("com.mysql.cj.jdbc.Driver");
			 con=DriverManager.getConnection("jdbc:mysql://192.168.1.251:3306/AXISMYINDIA?useSSL=false","bhanu","axis@123");			 
				...
				PreparedStatement ps2 =con.prepareStatement("SELECT DISTINCT(VALUE)  FROM `SAVE_SURVEY` WHERE `CHECKPOINT_ID` LIKE ? AND `DATETIME` > ?");
   ...
   <div class="container">	
        <form action="casteSelectVS.jsp" method="post">
            <label for="frName">Select a FR Name:</label>
            <select id="frName" name="frName">
            
			<%
			    for (String frValue:arr) {
			%>
			        <option value="<%=frValue%>"><%=frValue%></option>
